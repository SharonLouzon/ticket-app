...
public class TicketController {
	...
	/**
	 * share ticket by given ticket's data
	 * 
	 * @param authHeader - auth token
	 * @param ticket, String reciverEmail, String publicKeyAsString
	 * @return
	 */
	@PostMapping(path = "share")
	public ResponseEntity<?> share(
			@RequestHeader(value = "Authorization", required = false) String authHeader,
			@RequestBody Ticket ticket, @RequestBody String reciverEmail, @RequestBody String publicKeyAsString) {

		try {
			Integer userId = jwtServices.jwtValidation(authHeader);

			return new ResponseEntity<Ticket>(
					ticketService.sendTicket(userId, ticket, reciverEmail, publicKeyAsString), HttpStatus.OK);
		} catch (Exception e) {
			Map<String, String> error = ExceptionGenerator
					.exceptionGenerate(e.getMessage());
	...
	@GetMapping(path = "recive/{id}")
	public ResponseEntity<?> reciveTicket(
			@RequestHeader(value = "Authorization", required = false) String authHeader,
			@PathVariable("id") int ticketId, @RequestBody String privateKeyAsString) {
		try {
			Integer userId = jwtServices.jwtValidation(authHeader);
			Ticket ticket = ticketService.reciveTicket(userId, ticketId, privateKeyAsString);
			return new ResponseEntity<Ticket>(ticket, HttpStatus.OK);
